        ...
        if var_acceso['tipo'] == TipoEnum.STRUCT:
            ...
            if self.parametro in valor_recuperado:
                ...
                if tipo_referencia == TipoEnum.ANY or tipo_referencia == valor_asignar['tipo']:
                    ...
                    self.resultado.add_error('Semantico', f"El parametro \"{self.parametro}\" del estruct solo puede contener valores de tipo: \"{tipo_referencia.value}\" y esta asignando un valor de tipo: \"{valor_asignar['tipo'].value}\"", self.linea, self.columna)
            else:
