...
let a = [
          [ random[1] * 3, 51, random[4] / 2, (random[3] * 10) % 7 ],
          [ 1, 2, 3, 4 ]
        ];
...
function printMatriz(matrix: number[]) {
  console.log("[");
...
function sumarMatrices(matrix1: number[], matrix2: number[]): number[] {
...
function compararMatrices(matrix1: number[], matrix2: number[]): boolean {
  if (length(matrix1) !== length(matrix2)) { // Los length pueden manejarlos como arreglo.length
    return false;
  };
